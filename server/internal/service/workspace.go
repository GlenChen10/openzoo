...
func (s *WorkspaceService) List(ctx context.Context) ([]Workspace, error) {
	rows, err := s.db.Query(ctx, `SELECT id, name, slug, description, context, issue_prefix, created_at, updated_at FROM workspaces ORDER BY created_at DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var w Workspace
		if err := rows.Scan(&w.ID, &w.Name, &w.Slug, &w.Description, &w.Context, &w.IssuePrefix, &w.CreatedAt, &w.UpdatedAt); err != nil {
			return nil, err
		}
		ws = append(ws, w)
	}
...
func (s *WorkspaceService) Get(ctx context.Context, id string) (*Workspace, error) {
	var w Workspace
	err := s.db.QueryRow(ctx, `SELECT id, name, slug, description, context, issue_prefix, created_at, updated_at FROM workspaces WHERE id = $1`, id).
		Scan(&w.ID, &w.Name, &w.Slug, &w.Description, &w.Context, &w.IssuePrefix, &w.CreatedAt, &w.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &w, nil
