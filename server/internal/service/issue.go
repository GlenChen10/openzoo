...
func (s *IssueService) Create(ctx context.Context, workspaceID, title, description, status, priority string) (*Issue, error) {
	id := uuid.New().String()
	now := time.Now()
	...
	_, err = s.db.Exec(ctx, `INSERT INTO issues (id, workspace_id, number, identifier, title, description, status, priority, creator_type, creator_id, created_at, updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'member','system',$9,$9)`,
		id, workspaceID, number, identifier, title, description, status, priority, now)
	if err != nil {
		return nil, err
	}
